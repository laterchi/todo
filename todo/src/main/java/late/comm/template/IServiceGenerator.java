 ...
 * @fileName IServiceGenerator.java
 * @author chijingjia
 * @createTime :2018年11月21日 下午1:36:51
 * @version: v1.0
 */
@Slf4j
public class IServiceGenerator extends CodeGenerator {
	private static final String TEMPLATE_URL = "codetemplates/IService.template";
	...
	public static void generate(Class<? extends BaseEntity> clazz) {
		...
		try {
			...
			packageName =  packageName.substring(0, packageName.length() - ".entity".length()) + ".service";
			...
			String outputFileName = new StringBuilder("src/main/java/")//
					...
					.append("I").append(nickName).append("Service")//
					.append(".java")//
					.toString();
